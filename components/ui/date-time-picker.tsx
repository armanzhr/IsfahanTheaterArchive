...
import { type Locale, faIR } from "date-fns-jalali/locale";
import {
...
function genYears(yearRange = 50) {
  ...
  return Array.from({ length: yearRange * 2 + 1 }, (_, i) => ({
    value: today.jYear() - yearRange + i,
    label: (today.jYear() - yearRange + i).toString(),
  }));
}
...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    let locale: Pick<Locale, "options" | "localize" | "formatLong"> = faIR;
